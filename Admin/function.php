...
function add_user(){
	...
	{	$query="select * from user where user_name='$user_name' and password=md5('$password')";
		...
		if($count>0){
						...
						$query="insert into user(user_name,password,role)";
...
<?php
	function insert_category(){
		...
		elseif($cat_name!="")
		{
			...
			else
			{
				...
				else{echo "<script> window.alert('successfully inserted') </script>";}
			}
		}
	}
	...
	function add_product()
	{
		...
		elseif($product_name!=""&&$photo!="")
		{
			...
			else{$query="insert into product(product_name,category_id,price,qty,photo)";
		...
		function update_product()
		{
			...
			if(!$photo)
			{
				...
				$query.="price='$price',qty='$qty' where product_id='$product_id'";	
			}else{
				$query="update product set product_name='$product_name',category_id='$cat_id',";
				$query.="price='$price',qty='$qty',photo='$photo'where product_id='$product_id'";
			}
			$go_query=mysqli_query($connection,$query);
			if(!$go_query)
			{
